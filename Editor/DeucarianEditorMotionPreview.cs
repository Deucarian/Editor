...
{
    public enum DeucarianEditorPreviewTransition { None, Fade, Scale, Slide }

    /// <summary>Isolated editor motion specimen. It never drives a scene object or application state.</summary>
    public sealed class DeucarianEditorMotionPreview : VisualElement, IDisposable
    {
        ...
        public DeucarianEditorPreviewTransition Enter { get; set; } = DeucarianEditorPreviewTransition.Fade;
        public DeucarianEditorPreviewTransition Exit { get; set; } = DeucarianEditorPreviewTransition.Fade;
        public float EnterSeconds { get; set; } = 0.25f;
        public float ExitSeconds { get; set; } = 0.25f;
        public bool ReducedMotion { get; set; }

        public DeucarianEditorMotionPreview()
        {
        ...
        private void Tick()
        {
            ...
            else if (elapsed < enter + 0.8f) Apply(DeucarianEditorPreviewTransition.None, 1);
            else if (elapsed < enter + 0.8f + exit) Apply(Exit, 1 - (elapsed - enter - 0.8f) / exit);
            else if (elapsed < enter + 1.05f + exit) Apply(DeucarianEditorPreviewTransition.Fade, 0);
            else Stop();
        }

        private void Apply(DeucarianEditorPreviewTransition transition, float progress)
        {
            float value = Mathf.SmoothStep(0, 1, Mathf.Clamp01(progress));
            Specimen.style.opacity = transition == DeucarianEditorPreviewTransition.Fade ? value : 1;
            Specimen.transform.scale = Vector3.one * (transition == DeucarianEditorPreviewTransition.Scale ? value : 1);
            Specimen.transform.position = transition == DeucarianEditorPreviewTransition.Slide ? new Vector3((value - 1) * 120, 0, 0) : Vector3.zero;
        }
        ...
        public void Stop()
        {
            playing = false;
            animation.Pause();
            Apply(DeucarianEditorPreviewTransition.None, 1);
        }
